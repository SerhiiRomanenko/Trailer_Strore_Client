...
const AccordionItem: React.FC<{ title: string, children: ReactNode, defaultOpen?: boolean }> = ({ title, children, defaultOpen = true }) => {
...
const Filters: React.FC<FiltersProps> = ({ filters, onFilterChange, onResetFilters, allBrands, allSuspensionTypes }) => {
  ...
  return (
    <div className="bg-white p-5 rounded-xl border border-slate-200">
      <div className="flex justify-between items-center mb-4">
        ...
        <button onClick={onResetFilters} className="text-sm font-medium text-orange-600 hover:underline">
          Скинути
        </button>
      </div>
      ...
      <AccordionItem title="Бренд">
        <div className="space-y-2 max-h-48 overflow-y-auto pr-2">
          {allBrands.map(brand => (
            <label key={brand} className="flex items-center space-x-3 cursor-pointer p-1 rounded hover:bg-slate-100">
              <input
                type="checkbox"
      ...
      <AccordionItem title="Тип підвіски">
        <div className="space-y-2">
          {allSuspensionTypes.map(type => (
            <label key={type} className="flex items-center space-x-3 cursor-pointer p-1 rounded hover:bg-slate-100">
              <input
                type="checkbox"
